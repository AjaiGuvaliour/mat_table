...
export class CustomTableComponent implements OnInit ,AfterViewInit{
 ...
 if(keys[i]==""){option= this.renderer.createText("please select__")}
 else{option= this.renderer.createText(keys[i])};
 option.value = data[keys[i]];
 this.renderer.appendChild(matoption,option);
 this.renderer.appendChild(matselect1,matoption);
 }
 this.renderer.listen(matselect1, "change", () => {this.selectChange(typeof(data[matselect1.value]),document.getElementById(`${matselect1.id}`).parentElement.parentElement.nextSibling.firstChild.firstChild)}); 
 ...
 button.type="submit";
 this.renderer.setAttribute(button,"class","button_add column_delete mat-button-try");
 ...
 this.renderer.appendChild(this.dynamicSearchOption.nativeElement,overall_div);
 this.searchIndex ++;
 }
 ...
 case 'string':
 while (select_element.firstChild) select_element.removeChild(select_element.firstChild); 
 ...
 let fields = this.fiterdata[0],items=this.fiterdata ;
 ...
 if (searching_list[i][prop] && item[prop].toLowerCase().endsWith(searching_list[i][prop].toLowerCase()) - 1) {
 return false;
 }
 }
 else if(searching_list[i]["types"]=="Start With"){
 if (searching_list[i][prop] && item[prop].toLowerCase().startsWith(searching_list[i][prop].toLowerCase()) - 1) {
 return false;
 }
 }
 else if(searching_list[i]["types"]=="Equals"){
 if (searching_list[i][prop] && item[prop].toLowerCase() !== searching_list[i][prop].toLowerCase()) {
 return false;
 }
 }
 else if(searching_list[i]["types"]=="<"){
 if (searching_list[i][prop] && searching_list[i][prop] - 1 < item[prop]) {
 return false;
 }
 }
 else if(searching_list[i]["types"]==">"){
 if (searching_list[i][prop] && parseInt(searching_list[i][prop]) + 1 > item[prop]) {
 return false;
 }
 }
 else if(searching_list[i]["types"]=="<="){
 if (searching_list[i][prop] && parseInt(searching_list[i][prop]) < item[prop]) {
 return false;
 }
 }
 else if(searching_list[i]["types"]==">="){
 ...
 let matoption = this.renderer.createElement("option");
 if(keyValue[i]==" "){option= this.renderer.createText("please select__")}
 else{option= this.renderer.createText(keyValue[i])};
 ...
 this.renderer.appendChild(overall_div,row);
 this.renderer.appendChild(overall_div,padding);
 this.renderer.appendChild(this.dynamicSortOption.nativeElement,overall_div);
 this.sortIndex ++;
 }
 ...
 var element={}, type,selection,searched ;
 if(document.getElementById(`${"type"+i}`)){type = document.getElementById(`${"type"+i}`)["value"];} else{type=''};
 if(document.getElementById(`${"selection"+i}`)){selection = document.getElementById(`${"selection"+i}`)["value"];} else{selection=''};
 if(document.getElementById(`${"searched"+i}`)){searched = document.getElementById(`${"searched"+i}`)["value"];} else{searched=''};
 ...
 this.renderer.removeChild(this.dynamicSearchOption.nativeElement,selectedrow);
 }
 }
 this.searchIndex=0;
...
//  this.tableDatas=this.tabledata_cpy2.slice(0);
 ...
 let selectedrow=document.getElementById("rowSort"+i).parentElement;
 this.renderer.removeChild(this.dynamicSortOption.nativeElement,selectedrow);
 }
 }
 this.sortIndex=0;
 this.orderBy({});
